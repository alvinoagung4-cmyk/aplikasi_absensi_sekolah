...
// ============ IMPORT ROUTES ============
const kelasRoutes = require('./routes/api-kelas');
const siswaRoutes = require('./routes/api-siswa');
...
app.use(cors({
  origin: function(origin, callback) {
    ...
    const allowedOrigins = [
      ...
      process.env.NGROK_URL || 'https://cheliform-unappreciably-allison.ngrok-free.dev', // ngrok
      process.env.FRONTEND_URL || 'https://web-production-0a0ad.up.railway.app' // Railway (jika ada)
    ];

    // Allow any localhost origin (any port) for local testing
...
if (process.env.DATABASE_URL) {
  ...
  pool = new Pool({
    ...
    database: process.env.DB_NAME || 'absensingrok',
...
const sendVerificationEmail = async (email, nama_lengkap, token) => {
  ...
  const frontendBase = process.env.FRONTEND_URL || 'https://web-production-0a0ad.up.railway.app';
  const verificationLink = `${frontendBase.replace(/\/$/, '')}/verify-email?token=${token}`;
  ...
  try {
    ...
    if (result.rows.length === 0) {
      return res.status(401).json({ 
        success: false, 
        message: 'Nama lengkap/Email tidak ditemukan' 
      });
    }
...
// ============ REGISTER API ROUTES ============
// Pass pool to routes
kelasRoutes.setPool(pool);
siswaRoutes.setPool(pool);
